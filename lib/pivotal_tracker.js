...
PivotalTracker.prototype.moveStory = function(projectID, storyID, otherStoryID, moveBefore, token, callback) {
  var query = querystring.stringify({'move[move]': moveBefore ? 'before' : 'after', 'move[target]': otherStoryID});
  var relativePath = '/projects/' + projectID + '/stories/' + storyID + '/moves?' + query;
  this._parsedResponse('POST', relativePath, token, null, null, callback);
};
